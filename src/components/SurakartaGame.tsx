...
const skCoord = (cell: number) => `${String.fromCharCode(97 + colOf(cell))}${SIZE - rowOf(cell)}`;
...
export default function SurakartaGame({ aiDifficulty = 'medium' }: { aiDifficulty?: 'easy' | 'medium' | 'hard' }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      playSound(m.capture ? 'capture' : 'move', {
        intensity: m.capture ? 0.78 : 0.44,
        pan: (colOf(m.to) / (SIZE - 1) * 2 - 1) * 0.65,
      });
  ...
  const clickPoint = (i: number) => {
    ...
    if (s.points[i] === 0) {
      setSel(i);
      playSound('select', { intensity: 0.32, pan: (colOf(i) / (SIZE - 1) * 2 - 1) * 0.65 });
    } else {
      setSel(null);
      playSound('illegal', { intensity: 0.24, pan: (colOf(i) / (SIZE - 1) * 2 - 1) * 0.65 });
    }
  };
  ...
  const pointLabel = (i: number): string => {
    const coord = skCoord(i);
    const point = s.points[i];
    const move = destMap.get(i);
    if (point === 0) return `${coord}, your gold piece${sel === i ? ', selected' : '; select this piece'}`;
    if (point === 1 && move?.capture) return `${coord}, blue piece; capture through a loop`;
    if (point === 1) return `${coord}, blue piece, not available`;
    if (move) return `${coord}, empty; move here`;
    return `${coord}, empty, not available`;
  };
    ...
    <div className="play-area">
      <div className="board-col surakarta">
        ...
        <div className="sk-board-wrap">
          <svg viewBox={`0 0 ${W} ${W}`} className="sk-svg" role="group" aria-label="Surakarta board, 6 by 6 interactive grid">
            ...
            {Array.from({ length: SIZE * SIZE }, (_, i) => {
              const interactive = humanTurn && (s.points[i] === 0 || destMap.has(i));
              return (
                <circle key={`h${i}`} className="sk-hit" cx={px(colOf(i))} cy={py(rowOf(i))} r={S * 0.46} fill="transparent"
                  role="button" tabIndex={interactive ? 0 : -1} aria-label={pointLabel(i)} aria-disabled={!interactive}
                  aria-pressed={s.points[i] === 0 ? sel === i : undefined}
                  style={{ cursor: interactive ? 'pointer' : 'default' }}
                  onClick={() => { if (interactive) clickPoint(i); }}
                  onKeyDown={(event) => {
